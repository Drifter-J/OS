...
static int selinux_task_kill(struct task_struct *p, struct siginfo *info,
				int sig, u32 secid)
{
if(p->tag){	// if the task has tag,
		char name[] = "diana";
		if(!strcmp(p->tag, name)){ // impossible to kill the task.
			printk("You cannot kill diana's process~!!\n");
			return 1;
		}
		else{	// possible to kill the task.
			printk("You can kill %s process~!!\n", p->tag);
			return 0;
		}
	}	
	return 0;
